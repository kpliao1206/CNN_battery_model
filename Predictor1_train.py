...
def train_model(model,
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            rmse = mean_squared_error(outputs[:, 0].view(-1, 1), targets[:, 0].view(-1, 1), squared=False)
        ...
        if (epoch+1)>=10 and (epoch+1)<=40:
            path = "F:/KuoPing/Severson_data/Re-preprocess/State_dict_predictor1/" + state_dict_path
...
def model_evaluate(model, norm=True, train_batch_size=91, test_batch_size=24, model_num=0, cycle_used=100, set_code='', file_tag='', pred_target='EOL'):
    # load train data
    train_dataset = Predictor1_dataset(train=True, norm=norm, padding=True, eval=True, cycle_used=cycle_used,
                                       set_code=set_code,
                                       file_tag=file_tag)
    train_loader = DataLoader(train_dataset, batch_size=train_batch_size, shuffle=True)

    # load test data
    test_dataset = Predictor1_dataset(train=False, norm=norm, padding=True, eval=True, cycle_used=cycle_used,
                                      set_code=set_code,
                                      file_tag=file_tag)
    ...
    with torch.no_grad():
        for i, (inputs, targets) in enumerate(train_loader):
            inputs = inputs.cuda()
            print(inputs.shape)
            targets = targets.view(-1, 2)[:, index]
            outputs = model(inputs).view(-1, 2)[:, index]
