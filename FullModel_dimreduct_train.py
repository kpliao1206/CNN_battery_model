...
def dimreduct_eol_train(model, eol_train_loader, eol_test_loader,
                        num_epochs=500,
                        lr=1e-3,
                        weight_decay=1e-3,
                        cosine_period=20,
    ...
    for epoch in range(num_epochs):
        ...
        # save state_dict
        # 創建資料夾
        if (epoch+1)>=20 and (epoch+1)<=60:
            path = "F:/KuoPing/Severson_data/Re-preprocess/Full_model/Saved_"+trained_model+"/"+saved_folder
...
def dimreduct_ct_train(model, ct_train_loader, ct_test_loader,
                       num_epochs=500,
                       lr=1e-3,
                       weight_decay=1e-3,
                       cosine_period=20,
...
def dimreduct_eol_eval(eol_model, segment='discharge', set_code=''):
    if torch.cuda.is_available():
        device = torch.device('cuda')
        print('cuda is available')

    # load train data
    eol_train_dataset = DimReduction_dataset(train=True, pred_target='EOL', norm=True, segment=segment, set_code=set_code)
    ...
    eol_test_dataset = DimReduction_dataset(train=False, pred_target='EOL', norm=True, segment=segment, set_code=set_code)
...
def dimreduct_ct_eval(eol_model, segment='discharge', set_code=''):
    if torch.cuda.is_available():
        device = torch.device('cuda')
        print('cuda is available')

    # load train data
    eol_train_dataset = DimReduction_dataset(train=True, pred_target='chargetime', segment=segment, norm=True, set_code=set_code)
    eol_train_loader = DataLoader(eol_train_dataset, batch_size=1, shuffle=True)

    # load test dataDataLoader(eol_train_dataset, batch_size=batch_size, shuffle=False)
    eol_test_dataset = DimReduction_dataset(train=False, pred_target='chargetime', segment=segment, norm=True, set_code=set_code)
    eol_test_loader = DataLoader(eol_test_dataset, batch_size=1, shuffle=True)
